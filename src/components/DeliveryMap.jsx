...
// rutear, puede faltar en pedidos recién Preparando Pedidos), o si el script de Google
// Maps no cargó.
export default function DeliveryMap({ motorizado, destino, className = "" }) {
  ...
  return (
    ...
    >
      ...
      >
        {ruta && (
          <DirectionsRenderer directions={ruta} options={RUTA_OPTIONS} />
        )}
        <Marker
          position={ubicacion}
          icon={motoIcon}
      ...
      <div className="absolute left-3 bottom-3 w-52 rounded-xl bg-white shadow-lg px-3 py-2.5 animate-fade-in-up">
        ...
        <p className="text-[11px] text-gray-400 mb-0">
          ...
          {formatoAntiguedad(ubicacion.actualizada_hace_segundos)}
        </p>
      </div>
    </div>
  );
