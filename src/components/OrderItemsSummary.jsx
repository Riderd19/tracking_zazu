...
function agruparArticulos(articulos) {
  ...
  for (const articulo of articulos) {
    ...
    if (posicion === undefined) {
      ...
      fila.colores.set(
        colorTrim,
        (fila.colores.get(colorTrim) ?? 0) + Number(cantidad),
      );
  ...
  return (
    <div className="py-4">
      <div className="flex items-start justify-between gap-2">
        ...
        <p className="mb-0 shrink-0 text-sm font-semibold text-violet-700">
          S/ {total.toFixed(2)}
        </p>
      </div>
...
export default function OrderItemsSummary({
  ...
  compactoConModal = false,
}) {
  // En una vista lateral inicia compacto; dentro de un modal puede solicitarse
  // abierto para mostrar el detalle completo desde el primer momento.
          ...
          <div className="flex items-center gap-3 border-b border-gray-100 pb-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mb-0 text-xs text-gray-500">
                Consulta productos, cantidades y variantes
              </p>
            </div>
          </div>
  ...
  return (
    <div className="w-full rounded-2xl border border-gray-100 bg-white p-5">
      ...
      <button
        type="button"
        onClick={() => {
          if (!siempreAbierto) setDetalleAbierto((v) => !v);
        }}
        className={`mt-3 flex w-full items-center justify-between gap-2 border-0 bg-transparent p-0 ${siempreAbierto ? "cursor-default" : "cursor-pointer"}`}
      >
