...
function construirHitos(estadoActual) {
  ...
  return [
    ...
    },
    {
      ...
      codigos: ["entregado", "devuelto", "reprogramado"],
      icon: resultado.icon,
    },
  ];
}
...
const ESTADOS_ESPECIALES = {
  ...
  no_entregado: {
    texto: "No se pudo entregar tu pedido",
    color: "#ef4444",
    icon: <CloseCircleOutlined />,
  },
};
  ...
  if (indiceActual === -1) {
    return (
      <p className="text-sm text-gray-400 mb-0">
        Todavía no hay novedades para este pedido.
      </p>
    );
  }

  return (
    // overflow-y-visible explícito: por spec, overflow-x distinto de
    // "visible" fuerza a overflow-y a comportarse como "auto" si no se
    // especifica, y eso recortaba el resplandor (box-shadow) del círculo
    // "actual" en su animación de pulso.
      ...
      <div className="flex items-start">
        {hitos.map((hito, i) => {
          ...
          const estado =
            i < indiceActual || (i === indiceActual && esUltimoHito)
              ...
              : i === indiceActual
                ? "actual"
                : "pendiente";
          // El historial casi nunca trae fecha por hito. Para "Pedido Registrado" (el
          ...
          return (
            <div
              key={hito.label}
              className="relative flex flex-1 flex-col items-center px-1 sm:px-4 min-w-16"
            >
              {i > 0 && (
                <span
              ...
              <p
                className="mt-2 mb-0 text-sm font-semibold text-center leading-5"
                style={{ color: "#374151" }}
              >
                {hito.label}
              </p>
              <p
                className="mb-0 text-xs font-normal text-center leading-4"
                style={{ color: "#374151" }}
              >
                {estado === "pendiente"
                  ? "Pendiente"
